        ...
        script {
          ...
          node {
            ...
            withCredentials([sshUserPrivateKey(credentialsId: 'oscap', keyFileVariable: 'identity', usernameVariable: 'userName')]) {
              remote.user = userName
              remote.identityFile = identity
              signature = sshCommand remote: remote,sudo: true, command: "g=\$(mktemp -d) && f=\$(mktemp) && img=\$(mktemp) && trap \"rm \$img;rm \$f;rm -rf \$g\" EXIT || exit 255;docker save -o \$e ${NEXUS_SERVER}/${REPO_NAME}:${IMAGE_TAG};"
