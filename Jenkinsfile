  ...
  stages {
    ...
    stage('Pull from Staging') {
      ...
      steps {
        ...
        echo "Artifact path is   ${S3_REPORT_LOCATION}/${VENDOR_PRODUCT}/${IMAGE_TAG}/${DATETIME_TAG}_${BUILD_NUMBER}"
          ...
          node {
            withCredentials([sshUserPrivateKey(credentialsId: 'oscap', keyFileVariable: 'identity', usernameVariable: 'userName')]) {
              ...
              stage('OpenSCAP Scan') {
                ...
                sshCommand remote: remote, command: "/usr/sbin/aws s3 cp /tmp/report-cve.html ${S3_REPORT_LOCATION}/${VENDOR_PRODUCT}/${IMAGE_TAG}/${DATETIME_TAG}_${BUILD_NUMBER}/openscap/report-cve.html"
                sshCommand remote: remote, command: "/usr/sbin/aws s3 cp /tmp/report.html ${S3_REPORT_LOCATION}/${VENDOR_PRODUCT}/${IMAGE_TAG}/${DATETIME_TAG}_${BUILD_NUMBER}/openscap/report.html"
