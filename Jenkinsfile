  ...
  stages {
    ...
    stage('Push to External Registry (TODO)') {
      ...
      steps {
        ...
        sh "g=\$(mktemp -d) && trap \"rm -rf \$g\" EXIT || exit 255;f=\$(mktemp) && trap \"rm \$f\" EXIT || exit 255; echo '${SIGNING_KEY}'  > \$f; gpg --homedir \$g --import --batch --passphrase ${SIGNING_KEY_PASSPHRASE} \$f ;gpg --pinentry-mode loopback --detach-sign --homedir \$g -o sig.gpg --armor --yes --batch --passphrase ${SIGNING_KEY_PASSPHRASE} sometext.txt;cat sig.gpg; rm -fr \$g; rm \$f; rm sig.gpg"
      } // steps
    } // stage
